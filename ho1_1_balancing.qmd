...
A reciprocating engine generates forces that are fundamentally harder to balance than rotating masses. The piston moves in one plane only, the problem cannot be solved by a single counterweight alone.
...
| Term                          | Name                   |
|-------------------------------|------------------------|
| $\cos\theta$                  | Primary force          |
| $\dfrac{r}{\ell}\cos 2\theta$ | Secondary force        |
...
| At $N_c$ | Resonance; amplitude theoretically unbounded |
...
In $N_{c_n}$, the subscript $n$ is the **mode number** — an integer
$(1, 2, 3, \ldots)$ identifying which lateral vibration mode is being
referred to.

- $N_{c_1}$ — **first critical speed**, corresponding to the fundamental
  (lowest) mode shape, where the shaft bows into a single half-wave.

- $N_{c_2}$ — **second critical speed**, higher in frequency, less likely to be encountered
  in normal operation.

- $N_{c_3}$ — **third critical speed**, and so on.

The relationship $N_{c_n} \propto n^2$ means the second critical speed is
roughly $4\times$ the first, and the third is roughly $9\times$ the first.
In practice, only $N_{c_1}$ is of primary design concern.
